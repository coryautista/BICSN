...
export class UsuariosRepository implements IUsuariosRepository {
  ...
  async create(data: CreateUsuarioData): Promise<Usuario> {
    await this.mssqlPool.request()
      .input('usuarioId', sql.UniqueIdentifier, data.usuarioId ?? null)
      ...
      .query(`
        ...
        VALUES(
          COALESCE(@usuarioId, NEWID()), @username, @email, @passwordHash, @passwordAlgo,
          @displayName, @photoPath, @idOrganica0, @idOrganica1, @idOrganica2, @idOrganica3
        )
      `);
    ...
    const selectResult = await this.mssqlPool.request()
      ...
      .query(`
        SELECT TOP 1
          ...
          updatedAt
        FROM auth.[user]
        WHERE (@usuarioId IS NOT NULL AND id = @usuarioId)
           OR (@usuarioId IS NULL AND username = @username)
        ORDER BY createdAt DESC
      `);
