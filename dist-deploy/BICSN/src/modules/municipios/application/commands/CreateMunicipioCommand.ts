...
export class CreateMunicipioCommand {
  ...
  async execute(input: CreateMunicipioInput, tx?: any): Promise<Municipio> {
    try {
      ...
      // La clave municipal se puede repetir entre estados, pero no dentro del mismo estado.
      const existingMunicipio = await this.municipioRepo.findByClave(input.estadoId, input.claveMunicipio);
  ...
  private validateInput(input: CreateMunicipioInput): void {
    ...
    const estadoRegex = /^\d{2}$/;
    if (!estadoRegex.test(estadoIdTrimmed)) {
      throw new MunicipioInvalidEstadoError('debe contener solo 2 dígitos');
    }
