...
import { Dependencia, DependenciaWithHijas, TipoDependencia } from '../../domain/entities/Dependencia.js';
import { getPool, sql } from '../../../../../db/mssql.js';
import { sql as sqlType } from '../../../../../db/context.js';

/**
 * Repository implementation para el módulo Dependencia
 * Implementa la interface del dominio con acceso directo a la base de datos
 */
export class DependenciaRepository implements IDependenciaRepository {
  async findAll(): Promise<Dependencia[]> {
    ...
    `);
    return r.recordset.map((row: any) => this.mapToDependencia(row));
  }

  async findById(dependenciaId: number): Promise<Dependencia | null> {
    ...
    if (!row) return null;
    return this.mapToDependencia(row);
  }

  async findByTipo(tipoDependencia: TipoDependencia): Promise<Dependencia[]> {
  ...
  async findHijas(dependenciaPadreId: number): Promise<Dependencia[]> {
  ...
  async getDependenciaWithHijas(dependenciaId: number): Promise<DependenciaWithHijas | null> {
    if (!dependenciaId || typeof dependenciaId !== 'number' || dependenciaId <= 0) {
      throw new Error('Invalid dependenciaId: must be a positive number');
    }
    const p = await getPool();
    ...
    const dependencia = {
      ...
      tipoDependencia: r.recordset[0].tipoDependencia as TipoDependencia,
      ...
      dependenciasHijas: r.recordset
        ...
        .map((row: any) => ({
          ...
          tipoDependencia: row.hijaTipoDependencia as TipoDependencia,
  ...
  async create(
    ...
    return {
      ...
      tipoDependencia: row.tipoDependencia as TipoDependencia,
  ...
  async delete(dependenciaId: number, tx?: sql.Transaction): Promise<number | null> {
    if (!dependenciaId || typeof dependenciaId !== 'number' || dependenciaId <= 0) {
      throw new Error('Invalid dependenciaId: must be a positive number');
    }
    const req = tx ? new sqlType.Request(tx) : (await getPool()).request();
  ...
  private mapToDependencia(row: any): Dependencia {
    return {
      id: row.id,
      nombre: row.nombre,
      descripcion: row.descripcion,
