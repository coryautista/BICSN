...
import { Indicador, IndicadorWithRelations } from '../../domain/entities/Indicador.js';
import { getPool, sql } from '../../../../../db/mssql.js';
import { sql as sqlType } from '../../../../../db/context.js';

export class IndicadorRepository implements IIndicadorRepository {
  async findAll(): Promise<IndicadorWithRelations[]> {
    ...
    `);
    return r.recordset.map((row: any) => this.mapToIndicadorWithRelations(row));
  }

  async findById(indicadorId: number): Promise<IndicadorWithRelations | null> {
    ...
    if (!row) return null;
    return this.mapToIndicadorWithRelations(row);
  }

  async findByPrograma(programaId: number): Promise<IndicadorWithRelations[]> {
  ...
  async findByLineaEstrategica(lineaEstrategicaId: number): Promise<IndicadorWithRelations[]> {
  ...
  async findByEje(ejeId: number): Promise<IndicadorWithRelations[]> {
  ...
  async delete(indicadorId: number, tx?: sql.Transaction): Promise<number | null> {
    if (!indicadorId || typeof indicadorId !== 'number' || indicadorId <= 0) {
      throw new Error('Invalid indicadorId: must be a positive number');
    }
    const req = tx ? new sqlType.Request(tx) : (await getPool()).request();
  ...
  private mapToIndicadorWithRelations(row: any): IndicadorWithRelations {
    return {
      id: row.id,
      idPrograma: row.idPrograma,
      nombre: row.nombre,
