...
interface GuardarRevisionParams {
  tarea: RevisionTarea;
  numeroConcepto: number;
  importes: ImportesRevision;
}

interface GuardarRevisionResultado {
...
export class RevisionRepository {
  ...
  async obtenerConceptosActivos(): Promise<number[]> {
    const resultado = await this.mssqlPool.request().query(`
      SELECT numeroConcepto
      FROM reportes.catalogoRevision
      WHERE activo = 1
      ORDER BY numeroConcepto;
    `);
    return resultado.recordset.map((row) => Number(row.numeroConcepto));
  }
  ...
  async calcularLiberacionPcpFondoAhorro(
    tarea: RevisionTarea
  ): Promise<{ importes: ImportesRevision; registros: number }> {
    const rows = await executeSafeQuery(`
      SELECT COUNT(*) AS REGISTROS,
