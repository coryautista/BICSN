...
import { PrestamosMedianoPlazoResponse } from '../../domain/entities/PrestamoMedianoPlazo.js';
import { AportacionFondoDomainError, AportacionFondoError } from '../../domain/errors.js';

export class GetPrestamosMedianoPlazoQuery {
  ...
  async execute(
    ...
    const logContext = {
      ...
      claveOrganica0,
      claveOrganica1,
      tipo: 'PRESTAMOS_MEDIANO_PLAZO'
    };

    console.log('[APORTACIONES_FONDOS] [PRESTAMOS_MEDIANO_PLAZO] Iniciando consulta', logContext);
    ...
    try {
      ...
      console.log('[APORTACIONES_FONDOS] [PRESTAMOS_MEDIANO_PLAZO] Validando acceso a claves orgánicas', logContext);
      ...
      console.log('[APORTACIONES_FONDOS] [PRESTAMOS_MEDIANO_PLAZO] Período obtenido', { ...logContext, periodo });

      // Obtener préstamos a mediano plazo ejecutando procedimiento AP_S_VIV
      console.log('[APORTACIONES_FONDOS] [PRESTAMOS_MEDIANO_PLAZO] Ejecutando procedimiento AP_S_VIV', { ...logContext, periodo });
      const prestamos = await this.aportacionFondoRepo.obtenerPrestamosMedianoPlazo(
        claves.clave0,
        claves.clave1,
        periodo
      );
      ...
      console.log('[APORTACIONES_FONDOS] [PRESTAMOS_MEDIANO_PLAZO] Consulta completada exitosamente', {
        ...logContext,
        periodo,
      ...
      const duration = Date.now() - startTime;
      console.error('[APORTACIONES_FONDOS] [PRESTAMOS_MEDIANO_PLAZO] Error en consulta', {
        ...logContext,
      ...
      throw new AportacionFondoDomainError(
        `Error al consultar préstamos a mediano plazo: ${error.message || 'Error desconocido'}`,
