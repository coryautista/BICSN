...
export async function findIndicadorAnualById(indicadorAnualId: number) {
...
export async function listIndicadoresAnualesByIndicador(indicadorId: number) {
...
export async function listIndicadoresAnualesByAnio(anio: number) {
...
export async function createIndicadorAnual(
  idIndicador: number,
  anio: number,
  enero?: number,
  febrero?: number,
...
export async function updateIndicadorAnual(
...
export async function deleteIndicadorAnual(indicadorAnualId: number, tx?: sqlType.Transaction) {
  if (!indicadorAnualId || typeof indicadorAnualId !== 'number' || indicadorAnualId <= 0) {
    throw new Error('Invalid indicadorAnualId: must be a positive number');
  }
  const req = tx ? new sqlType.Request(tx) : (await getPool()).request();
