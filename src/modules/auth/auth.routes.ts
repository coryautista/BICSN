...
import { denylistAccessToken } from './infrastructure/security/AuthTokenService.js';
import { requireAuth } from './auth.middleware.js';
import type { RegisterCommand } from './application/commands/RegisterCommand.js';
import type { LoginCommand } from './application/commands/LoginCommand.js';
import type { RefreshTokenCommand } from './application/commands/RefreshTokenCommand.js';
...
export default async function authRoutes(app: FastifyInstance) {
  app.post('/auth/register', {
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          idOrganica0: { type: ['string', 'integer'], pattern: '^[A-Za-z0-9]{1,2}$', minLength: 1, maxLength: 2 },
          idOrganica1: { type: ['string', 'integer'], pattern: '^[A-Za-z0-9]{1,2}$', minLength: 1, maxLength: 2 },
          idOrganica2: { type: ['string', 'integer'], pattern: '^[A-Za-z0-9]{1,2}$', minLength: 1, maxLength: 2 },
          idOrganica3: { type: ['string', 'integer'], pattern: '^[A-Za-z0-9]{1,2}$', minLength: 1, maxLength: 2 }
        }
      },
      response: {
        201: {
    ...
    try {
      // Resolve RegisterCommand from DI container
      const registerCommand = req.diScope.resolve<RegisterCommand>('registerCommand');
      const u = await registerCommand.execute({
        username: parsed.data.username,
        email: parsed.data.email,
        password: parsed.data.password,
        displayName: parsed.data.displayName,
        photoPath: parsed.data.photoPath,
        idOrganica0: parsed.data.idOrganica0,
        idOrganica1: parsed.data.idOrganica1,
        idOrganica2: parsed.data.idOrganica2,
        idOrganica3: parsed.data.idOrganica3
      });
  ...
  app.post('/auth/refresh', {
    ...
    try {
      ...
      // Resolve RefreshTokenCommand from DI container
      const refreshTokenCommand = req.diScope.resolve<RefreshTokenCommand>('refreshTokenCommand');
      const { newRefreshToken } = await refreshTokenCommand.execute({
        currentRefreshToken: token,
        ip,
        userAgent: ua
      });
      
      setCookie(reply, 'refresh_token', newRefreshToken, '/v1/auth');
