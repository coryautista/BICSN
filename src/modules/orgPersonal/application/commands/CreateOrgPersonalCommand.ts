...
import { OrgPersonal, CreateOrgPersonalData } from '../../domain/entities/OrgPersonal.js';
import {
...
export class CreateOrgPersonalCommand {
  ...
  async execute(data: CreateOrgPersonalData, userId?: string): Promise<OrgPersonal> {
    ...
    try {
      ...
      if (error instanceof OrgPersonalAlreadyExistsError ||
      ...
      console.error(`[ORG_PERSONAL] Error al crear registro orgPersonal:`, error);
      throw error;
    }
  }

  private validateCreateData(data: CreateOrgPersonalData): void {
    // Validar interno
    if (!data.interno || typeof data.interno !== 'number' || data.interno <= 0 || !Number.isInteger(data.interno)) {
      throw new OrgPersonalInvalidInternoError(data.interno);
    }
