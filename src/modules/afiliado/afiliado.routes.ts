...
import {
  ...
  CreateCambioSueldoSchema,
  CreateBajaPermanenteSchema,
  CreateBajaSuspensionSchema,
  CreateBajaTerminaSuspensionSchema,
  CreateBajaTerminaSuspensionYBajaSchema,
  AplicarBDIsspeaIndividualSchema,
  AplicarBDIsspeaLoteSchema,
  UpdateBitacoraAfectacionOrgTerminadoSchema,
  CargarSemanasExtemporaneasLoteSchema,
} from "./afiliado.schemas.js";
import { createAfiliadoAfiliadoOrgMovimientoService } from "./infrastructure/services/AfiliadoService.js";
import {
  cambiarAStatusAfiliado,
  cambiarStatusAfiliadosLote,
  getAfiliadoStatusHistory,
  getAfiliadosByStatus,
  getAfiliadosPendientes,
  getAllStatusControl,
} from "./infrastructure/services/AfiliadoStatusService.js";
import {
  getBitacoraAfectacionOrgPorOrganicaYPeriodo,
  getUltimaBitacoraAfectacionOrgPorOrganica,
...
import { GetHistorialMovimientosQuincenaSqlQuery } from "./application/queries/GetHistorialMovimientosQuincenaSqlQuery.js";
import { GetHistorialMovimientosQuincenaFirebirdQuery } from "./application/queries/GetHistorialMovimientosQuincenaFirebirdQuery.js";
...
function getTokenUserForMovimiento(user: any): {
  userId: number | null;
  userUid: string | null;
} {
...
async function resolverEntregaRendimiento(
  ...
  const pool = await getPool();
  const result = await pool.request().input("interno", sql.Int, interno).query(`
...
async function validarMotivoBaja(
  motivoBajaId: number | null | undefined,
  tipo: "bajaPermanente" | "suspension",
) {
  if (!motivoBajaId) {
  ...
  const result = await pool
    .request()
    .input("motivoBajaId", sql.Int, motivoBajaId).query(`
  ...
  if (!motivo || !(motivo.Activo === true || motivo.Activo === 1)) {
    throw new Error(
      "MOTIVO_BAJA_INVALID: motivoBajaId no existe o no esta activo",
    );
  }

  if (
    tipo === "bajaPermanente" &&
    !(motivo.AplicaBajaPermanente === true || motivo.AplicaBajaPermanente === 1)
  ) {
    throw new Error(
      "MOTIVO_BAJA_NO_APLICA: el motivo no aplica para baja permanente",
    );
  }

  if (
    tipo === "suspension" &&
    !(motivo.AplicaSuspension === true || motivo.AplicaSuspension === 1)
  ) {
    throw new Error(
      "MOTIVO_BAJA_NO_APLICA: el motivo no aplica para suspension",
    );
  }
}
...
export default async function afiliadoRoutes(app: FastifyInstance) {
  app.get(
    "/afiliado/historial-movimientos-quincena",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Consulta histórico de movimientos de afiliados en SQL Server por quincena. Incluye Afiliado, AfiliadoOrg, Movimiento y TipoMovimiento.",
        summary: "Histórico de movimientos por quincena",
        tags: ["afiliado", "historico-movimientos"],
        security: [{ bearerAuth: [] }],
        querystring: {
          type: "object",
          properties: {
            periodo: {
              type: "string",
              pattern: "^[0-9]{4}$",
              description: "Periodo formato QQAA, ejemplo 0626",
            },
            quincena: {
              type: "integer",
              minimum: 1,
              maximum: 24,
              description: "Quincena, alternativa a periodo",
            },
            anio: {
              type: "integer",
              minimum: 2000,
              maximum: 2099,
              description: "Año, alternativa a periodo",
            },
            org0: {
              type: "string",
              minLength: 1,
              ...
              description: "Orgánica 0 opcional; si no viene se toma del token",
            },
            org1: {
              type: "string",
              minLength: 1,
              maxLength: 2,
              pattern: "^[A-Za-z0-9]{1,2}$",
              description: "Orgánica 1 opcional; si no viene se toma del token",
            },
            buscar: {
              type: "string",
              maxLength: 100,
              description:
                "Búsqueda por nombre, apellidos, RFC, CURP, noEmpleado o interno",
            },
    ...
    async (req, reply) => {
      try {
        ...
        const getHistorial =
          req.diScope.resolve<GetHistorialMovimientosQuincenaSqlQuery>(
            "getHistorialMovimientosQuincenaSqlQuery",
          );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getHistorialMovimientosQuincena",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    "/afiliado/historial-movimientos-quincena-firebird",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Consulta movimientos de afiliados por periodo y orgánica en Firebird. Incluye PERSONA, ORG_PERSONAL e HISTORIAL_MOVIMIENTOS_QUIN_IND.",
        summary: "Histórico de movimientos por quincena Firebird",
        tags: ["afiliado", "historico-movimientos", "firebird"],
        security: [{ bearerAuth: [] }],
        querystring: {
          type: "object",
          required: ["periodo"],
          properties: {
            periodo: {
              type: "string",
              pattern: "^[0-9]{4}$",
              description: "Periodo formato QQAA, ejemplo 0526",
            },
            org0: {
              type: "string",
              minLength: 1,
            ...
            buscar: {
              ...
              maxLength: 100,
              description:
                "Búsqueda por interno, nombre, noEmpleado, RFC o movimiento",
            },
            page: { type: "integer", minimum: 1, default: 1 },
            pageSize: {
              type: "integer",
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getHistorialMovimientosQuincenaFirebird",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    "/afiliado/reset",
    {
      ...
      schema: {
        description:
          "Ejecutar stored procedure sp_ResetAfiliados para resetear afiliados. Requiere rol admin.",
    ...
    async (req, reply) => {
      try {
        ...
        const result = await p.request().execute("[afi].[sp_ResetAfiliados]");

        return reply.send(
          ok({
            message:
              "Stored procedure sp_ResetAfiliados ejecutado exitosamente",
            result: result.recordset || result.rowsAffected || {},
          }),
        );
      } catch (error: any) {
        return handleAfiliadoError(error, reply, {
          operation: "resetAfiliados",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    "/afiliado/semanas-extemporaneas",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Obtiene los registros de semanas extemporáneas (los mismos que se insertan en carga-semanas-extemporaneas) filtrados por org0, org1 y periodo (QnaAplica).",
      ...
      try {
        ...
        if (!org0 || org0.length !== 2) {
          return reply.code(400).send({
            ...
            error: {
              ...
              message: "org0 es requerido y debe tener 2 caracteres",
            },
          });
        }
        if (!org1 || org1.length !== 2) {
          return reply.code(400).send({
            ...
            error: {
              code: "VALIDATION_ERROR",
              message: "org1 es requerido y debe tener 2 caracteres",
            },
          });
        }
        if (isNaN(periodo) || periodo < 1) {
          return reply.code(400).send({
            ...
            error: {
              code: "VALIDATION_ERROR",
              message: "periodo es requerido y debe ser un número positivo",
            },
          });
        }

        const getSemanasExtemporaneasQuery =
          req.diScope.resolve<GetSemanasExtemporaneasQuery>(
            "getSemanasExtemporaneasQuery",
          );
        const registros = await getSemanasExtemporaneasQuery.execute({
          org0,
          org1,
          periodo,
        });
  ...
  app.post(
    "/afiliado/carga-semanas-extemporaneas",
    {
      preHandler: [requireAuth, requireRole("admin")],
      schema: {
        description:
          "Carga en lote de semanas extemporáneas. Valida que no existan duplicados en el lote ni en BD (Interno + QnaAplica). Solo rol admin.",
        ...
        body: {
          ...
          properties: {
            registros: {
              ...
              maxItems: 500,
              description:
                "Array de registros de semanas extemporáneas (máximo 500)",
              items: {
                type: "object",
                required: [
                  "qnaAplica",
                ...
                properties: {
                  qnaAplica: {
                    type: "integer",
                    minimum: 1,
                    description: "Quincena de aplicación (ej: 1025)",
                  },
                  interno: {
                    type: "integer",
                    minimum: 1,
                    description: "Número interno del afiliado (ej: 84741)",
                  },
                  org0: {
                    type: "string",
                    minLength: 2,
                    maxLength: 2,
                    description: "Código orgánico nivel 0 (ej: 02)",
                  },
                  org1: {
                    type: "string",
                    minLength: 2,
                    maxLength: 2,
                    description: "Código orgánico nivel 1 (ej: 01)",
                  },
                  org2: {
                    type: "string",
                    minLength: 2,
                    maxLength: 2,
                    description: "Código orgánico nivel 2 (ej: 00)",
                  },
                  org3: {
                    type: "string",
                    minLength: 2,
                    maxLength: 2,
                    description: "Código orgánico nivel 3 (ej: 00)",
                  },
                  qnasPlus: {
                    type: "integer",
                    minimum: 0,
                    description: "Quincenas adicionales (ej: 171)",
                  },
                  cair: {
                    type: "number",
                    minimum: 0,
                    description: "Monto CAIR (ej: 8309.85)",
                  },
                  fra: {
                    type: "number",
                    minimum: 0,
                    description: "Monto FRA (ej: 16620.45)",
                  },
                  fre: {
                    type: "number",
                    minimum: 0,
                    description: "Monto FRE (ej: 108634.93)",
                  },
                  fh: {
                    type: "number",
                    minimum: 0,
                    description: "Monto FH (ej: 1454.28)",
                  },
                  fv: {
                    type: "number",
                    minimum: 0,
                    description: "Monto FV (ej: 5817.27)",
                  },
                  faa: {
                    type: "number",
                    minimum: 0,
                    description: "Monto FAA (ej: 20774.97)",
                  },
                  fae: {
                    type: "number",
                    minimum: 0,
                    description: "Monto FAE (ej: 10387.77)",
                  },
                  usuario: {
                    type: "string",
                    minLength: 1,
                    maxLength: 50,
                    description: "Usuario que realiza la carga (ej: DACERO)",
                  },
                },
              },
            },
            ...
            properties: {
              ...
              data: {
                ...
                properties: {
                  ...
                  total: {
                    type: "integer",
                    description: "Total de registros en el lote",
                  },
                },
              },
            },
    ...
    async (req, reply) => {
      try {
        ...
        const cargarSemanasExtemporaneasLoteCommand =
          req.diScope.resolve<CargarSemanasExtemporaneasLoteCommand>(
            "cargarSemanasExtemporaneasLoteCommand",
          );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cargarSemanasExtemporaneas",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        const getAllAfiliadosQuery = req.diScope.resolve<GetAllAfiliadosQuery>(
          "getAllAfiliadosQuery",
        );
  ...
  app.get(
    "/afiliado/preview-dp-edita-entidad-lote",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Este endpoint solo acepta POST. Use POST en lugar de GET.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        response: {
          405: {
    ...
    async (_req, reply) => {
      return reply.code(405).send({
        ...
        error: {
          ...
          message:
            "Este endpoint solo acepta el método POST. Por favor, use POST en lugar de GET.",
        },
      });
    },
  );
  ...
  app.post(
    "/afiliado/preview-dp-edita-entidad-lote",
    {
      ...
      schema: {
        description:
          "Genera comandos SQL ejecutables para Firebird del stored procedure DP_EDITA_ENTIDAD para todos los afiliados en numValidacion=2 (Aprobado) o numValidacion=3 (En Revisión) de la orgánica del usuario. NO ejecuta nada, solo genera los comandos SQL.",
        ...
        body: {
          ...
          properties: {
            motivo: {
              type: "string",
              maxLength: 500,
              description: "Motivo (opcional, no se usa en preview)",
            },
            observaciones: {
              type: "string",
              maxLength: 1000,
              description: "Observaciones (opcional, no se usa en preview)",
            },
          },
      ...
      try {
        ...
        if (!userOrg0 || !userOrg1) {
          return reply
            ...
            .send(
              fail(
                "USER_ORGANICA_NOT_FOUND: Usuario no tiene orgánica configurada",
              ),
            );
        }
        ...
        const org0 =
          typeof userOrg0 === "string"
            ? userOrg0.padStart(2, "0")
            : String(userOrg0).padStart(2, "0");
        const org1 =
          typeof userOrg1 === "string"
            ? userOrg1.padStart(2, "0")
            : String(userOrg1).padStart(2, "0");

        // Obtener pool de conexión
        const p = await getPool();

        // Obtener todos los afiliados en estados 2 y 3
        const afiliadosQuery = await p
          .request()
          .input("org0", sql.VarChar(30), org0)
          .input("org1", sql.VarChar(30), org1).query(`
        ...
        const generarComandoEjecutable = (
          log: LogEjecucionDPEditaEntidad,
        ): string | null => {
          if (
            !log.listoParaEjecutar ||
            !log.datosPreparados ||
            !log.parametros
          ) {
            return null;
          ...
          const parametrosFormateados = log.parametros.map((param) =>
            formatearValor(param.valor, param.tipo),
          );
        ...
        for (const afiliado of afiliadosParaProcesar) {
          try {
            const nombreCompleto =
              `${afiliado.nombre || ""} ${afiliado.apellidoPaterno || ""} ${afiliado.apellidoMaterno || ""}`.trim();

            // 1. Verificar si necesita DP_EDITA_PERSONAL (no tiene interno)
            const resultadoInterno = await obtenerInternoParaPreview(
              afiliado.id,
            );
            ...
            if (resultadoInterno.logDPEditaPersonal) {
              ...
              comandosDPEditaPersonal.push({
                ...
                comandoEjecutable:
                  resultadoInterno.logDPEditaPersonal.comandoEjecutable,
                validaciones: resultadoInterno.logDPEditaPersonal.validaciones,
                listoParaEjecutar:
                  resultadoInterno.logDPEditaPersonal.listoParaEjecutar,
            ...
            for (const movimiento of movimientosActivos) {
              const log = await prepararLogEjecucionDPEditaEntidad(
                movimiento,
                org0,
                org1,
              );
              ...
              const listoParaEjecutar =
                log.listoParaEjecutar && !tieneErroresDPEditaPersonal;
              const errores = tieneErroresDPEditaPersonal
                ? [
                    ...log.errores,
                    "El afiliado requiere DP_EDITA_PERSONAL pero tiene errores",
                  ]
                : log.errores;
        ...
        return handleAfiliadoError(error, reply, {
          operation: "previewDPEditaEntidadLote",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const getAfiliadoByIdQuery = req.diScope.resolve<GetAfiliadoByIdQuery>(
          "getAfiliadoByIdQuery",
        );
        ...
        return handleAfiliadoError(error, reply, {
          ...
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
    },
  );
  ...
  app.get(
    "/afiliado/bitacora-afectacion/:afectacionId/terminado",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          'Actualizar el campo Accion a "TERMINADO" en BitacoraAfectacionOrg usando AfectacionId como parámetro',
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        params: {
          type: "object",
    ...
    async (req, reply) => {
      try {
        ...
        const updateBitacoraCommand =
          req.diScope.resolve<UpdateBitacoraAfectacionOrgTerminadoCommand>(
            "updateBitacoraAfectacionOrgTerminadoCommand",
          );
        ...
        if (error.message && error.message.includes("No se encontró")) {
          return reply.code(404).send({
            ...
            error: {
              ...
              message:
                error.message ||
                "No se encontró registro con el AfectacionId proporcionado",
            },
          });
        }
        return handleAfiliadoError(error, reply, {
          ...
          user: req.user?.sub,
          afectacionId: (req.params as any)?.afectacionId,
        });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      try {
        const createAfiliadoCommand =
          req.diScope.resolve<CreateAfiliadoCommand>("createAfiliadoCommand");
  ...
  app.get(
    "/afiliado/complete",
    {
      ...
      schema: {
        description:
          "Ruta no permitida. Use POST /afiliado/complete para crear un afiliado completo.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        response: {
          405: {
    ...
    async (_req, reply) => {
      return reply
        ...
        .send(
          fail(
            "METHOD_NOT_ALLOWED: Use POST /afiliado/complete para crear un afiliado completo",
          ),
        );
    },
  );
  ...
  app.put(
    ...
    async (req, reply) => {
      ...
      try {
        const updateAfiliadoCommand =
          req.diScope.resolve<UpdateAfiliadoCommand>("updateAfiliadoCommand");
        const record = await updateAfiliadoCommand.execute({
          id,
          ...parsed.data,
        });
        return reply.send(ok(record));
  ...
  app.post(
        ...
        body: {
          ...
          properties: {
            ...
            numQuinquenios: {
              ...
              minimum: 0,
              nullable: true,
              description: "Número de quinquenio (0, 1, 2, 3...)",
            },
            ...
            categoriaPuestoOrgId: {
              ...
              nullable: true,
              description:
                "Categoría de puesto usada para llenar sueldo en nómina al alta",
            },
            ...
            periodo: {
              type: "string",
              pattern: "^(0[1-9]|1\\d|2[0-4])\\d{2}$",
              description:
                "Periodo de captura en formato QQAA, por ejemplo 1526",
            },
            fechaMovimiento: {
              type: "string",
              format: "date",
              description:
                "Fecha efectiva del alta para calcular días laborados",
            },
            ...
            entregaRendimiento: {
              ...
              enum: ["Si", "No"],
              nullable: true,
              description: "Indica si se entrega rendimiento",
            },
          },
        },
        response: {
    ...
    async (req, reply) => {
      const parsed = CreateAfiliadoAfiliadoOrgMovimientoSchema.safeParse(
        req.body,
      );
      if (!parsed.success) {
        return reply.code(400).send(fail(parsed.error.message));
      ...
      try {
        if (!parsed.data.fechaMovimiento) {
          return reply
            ...
            .send(
              fail(
                "FECHA_MOVIMIENTO_REQUIRED: fechaMovimiento es requerida para alta",
              ),
            );
        }
        if (!(await puedeCapturarMovimiento())) {
          return reply
            ...
            .send(
              fail(
                "No existe una ventana ALTA_BAJA_CAMBIO o BA_MOVIMIENTO activa para la fecha actual de captura.",
                "MOVIMIENTO_FUERA_DE_VENTANA",
              ),
            );
        }
        if (!parsed.data.categoriaPuestoOrgId) {
          return reply
            ...
            .send(
              fail(
                "CATEGORIA_PUESTO_ORG_REQUIRED: categoriaPuestoOrgId es requerida para alta",
              ),
            );
        }
        ...
        const claveOrganica2 =
          parsed.data.claveOrganica2 ?? req.user?.idOrganica2 ?? null;
        const claveOrganica3 =
          parsed.data.claveOrganica3 ?? req.user?.idOrganica3 ?? null;

        // Construir orgs1-4: si se envían en el body, se usan esos valores
        // Si no, se construyen concatenando las claveOrganica
        const orgs1 =
          parsed.data.orgs1 !== undefined && parsed.data.orgs1 !== null
            ? parsed.data.orgs1
            : [claveOrganica0, claveOrganica1].filter(Boolean).join("");
        const orgs2 =
          parsed.data.orgs2 !== undefined && parsed.data.orgs2 !== null
            ? parsed.data.orgs2
            : [claveOrganica0, claveOrganica1, claveOrganica2]
                .filter(Boolean)
                .join("");
        const orgs3 =
          parsed.data.orgs3 !== undefined && parsed.data.orgs3 !== null
            ? parsed.data.orgs3
            : [claveOrganica0, claveOrganica1, claveOrganica2, claveOrganica3]
                .filter(Boolean)
                .join("");
        const orgs4 =
          parsed.data.orgs4 !== undefined && parsed.data.orgs4 !== null
            ? parsed.data.orgs4
            : [claveOrganica0, claveOrganica1, claveOrganica2, claveOrganica3]
                .filter(Boolean)
                .join("");

        const createCompleteAfiliadoCommand =
          req.diScope.resolve<CreateCompleteAfiliadoCommand>(
            "createCompleteAfiliadoCommand",
          );
        const result = await createCompleteAfiliadoCommand.execute({
          afiliado: {
            ...
            domicilioCalle: parsed.data.domicilioCalle ?? null,
            domicilioNumeroExterior:
              parsed.data.domicilioNumeroExterior ?? null,
            domicilioNumeroInterior:
              parsed.data.domicilioNumeroInterior ?? null,
            domicilioEntreCalle1: parsed.data.domicilioEntreCalle1 ?? null,
            domicilioEntreCalle2: parsed.data.domicilioEntreCalle2 ?? null,
            domicilioColonia: parsed.data.domicilioColonia ?? null,
            domicilioCodigoPostal: parsed.data.domicilioCodigoPostal ?? null,
            ...
            celular: parsed.data.celular ?? null,
            expediente: parsed.data.expediente ?? null,
            quincenaAplicacion: parsePeriodoMovimiento(parsed.data.periodo)
              .quincena,
            anioAplicacion: parsePeriodoMovimiento(parsed.data.periodo).anio,
            codigoPostal: parsed.data.domicilioCodigoPostal ?? null,
          },
          afiliadoOrg: {
            nivel0Id: null,
            ...
            activo: true,
            fechaMovAlt: parsed.data.fechaMovimiento,
  ...
  app.post(
        ...
        body: {
          ...
          properties: {
            ...
            expediente: {
              ...
              nullable: true,
              description:
                "Identificador de expediente. Si no se proporciona, se utilizará la CURP como identificador de documentos en el FTP.",
            },
            // AfiliadoOrg fields
            internoOrg: { type: "number", nullable: true },
            sueldo: { type: "number" },
            ...
            fechaMovimiento: {
              type: "string",
              format: "date",
              description: "Fecha efectiva del cambio; no afecta nómina",
            },
            observaciones: { type: "string", maxLength: 1024, nullable: true },
            entregaRendimiento: {
              type: "string",
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const validateInternoInFirebirdQuery =
        ...
        if (!internoExists) {
          return reply
            ...
            .send(
              fail(
                "INTERNO_NOT_FOUND_IN_FIREBIRD: El número de interno no existe en las tablas PERSONAL y ORG_PERSONAL",
              ),
            );
        }
        ...
        body: {
          ...
          properties: {
            ...
            fechaMovimiento: {
              ...
              format: "date",
              description:
                "Fecha efectiva de la baja para calcular días laborados",
            },
            observaciones: { type: "string", maxLength: 1024, nullable: true },
            entregaRendimiento: {
              type: "string",
            ...
            motivoBajaId: {
              ...
              nullable: true,
              description:
                "Motivo de baja seleccionado del catalogo afi.CatalogoMotivoBaja",
            },
          },
        },
        response: {
      ...
      try {
        if (!parsed.data.fechaMovimiento) {
          return reply
            ...
            .send(
              fail(
                "FECHA_MOVIMIENTO_REQUIRED: fechaMovimiento es requerida para baja permanente",
              ),
            );
        }
        if (!(await puedeCapturarMovimiento())) {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "createBajaPermanente",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    "/afiliado/baja-suspension",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Crear movimiento de baja suspensión de afiliación (tipoMovimientoId=3). Similar a /afiliado/complete pero para suspensiones de afiliación. NOTA: claveOrganica0 y claveOrganica1 se obtienen automáticamente del usuario autenticado. claveOrganica2 y claveOrganica3 son opcionales y si se envían en el body se usan esos valores, si no se obtienen del usuario autenticado. orgs1-orgs4 son opcionales y si se envían en el body se usan esos valores, si no se construyen automáticamente concatenando las claveOrganica. El campo expediente es opcional y si no se proporciona se utilizará la CURP como identificador de documentos en el FTP.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        body: {
          type: "object",
          ...
          properties: {
            ...
            fechaMovimiento: {
              type: "string",
              format: "date",
              description: "Fecha efectiva de la suspensión",
            },
            observaciones: { type: "string", maxLength: 1024, nullable: true },
            entregaRendimiento: {
              type: "string",
    ...
    async (req, reply) => {
      ...
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "createBajaSuspension",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    "/afiliado/baja-termina-suspension",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Crear movimiento de baja termina suspensión de afiliación (tipoMovimientoId=4). Similar a /afiliado/complete pero para terminación de suspensiones de afiliación. NOTA: claveOrganica0 y claveOrganica1 se obtienen automáticamente del usuario autenticado. claveOrganica2 y claveOrganica3 son opcionales y si se envían en el body se usan esos valores, si no se obtienen del usuario autenticado. orgs1-orgs4 son opcionales y si se envían en el body se usan esos valores, si no se construyen automáticamente concatenando las claveOrganica. El campo expediente es opcional y si no se proporciona se utilizará la CURP como identificador de documentos en el FTP.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        body: {
          type: "object",
          ...
          properties: {
            ...
            fechaMovimiento: {
              type: "string",
              format: "date",
              description: "Fecha efectiva de la terminación de suspensión",
            },
            observaciones: { type: "string", maxLength: 1024, nullable: true },
            entregaRendimiento: {
              type: "string",
    ...
    async (req, reply) => {
      ...
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "createTerminarSuspension",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    "/afiliado/baja-termina-suspension-y-baja",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Crear movimiento de baja termina suspensión de afiliación y baja (tipoMovimientoId=6). Similar a /afiliado/complete pero para terminación de suspensiones de afiliación y baja simultánea. NOTA: claveOrganica0 y claveOrganica1 se obtienen automáticamente del usuario autenticado. claveOrganica2 y claveOrganica3 son opcionales y si se envían en el body se usan esos valores, si no se obtienen del usuario autenticado. orgs1-orgs4 son opcionales y si se envían en el body se usan esos valores, si no se construyen automáticamente concatenando las claveOrganica. El campo expediente es opcional y si no se proporciona se utilizará la CURP como identificador de documentos en el FTP.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        body: {
          type: "object",
          ...
          properties: {
            ...
            fechaMovimiento: {
              ...
              format: "date",
              description:
                "Fecha efectiva de la baja combinada para calcular días laborados",
            },
            observaciones: { type: "string", maxLength: 1024, nullable: true },
            entregaRendimiento: {
              type: "string",
    ...
    async (req, reply) => {
      ...
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "createTerminarSuspensionYBaja",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    "/afiliado/obtener-movimientos-quincenales",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          'Obtener listado de afiliados, afiliadosOrg y movimientos quincenales filtrados por org0 y org1 del usuario autenticado, donde estatus de afiliado = 1 y estatus de movimientos = "A". Omite campos de control y auditoria.',
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        response: {
          200: {
      ...
      try {
        ...
        if (!periodo) {
          return reply
            ...
            .send(
              fail(
                "PERIODO_NOT_FOUND: No se pudo determinar el periodo actual para la orgánica del usuario",
              ),
            );
        }
        ...
        const getMovimientosQuincenalesQuery =
          req.diScope.resolve<GetMovimientosQuincenalesQuery>(
            "getMovimientosQuincenalesQuery",
          );
        const movimientos = await getMovimientosQuincenalesQuery.execute(
          userOrg0,
          userOrg1,
          quincenaId,
        );
  ...
  app.delete(
    ...
    async (req, reply) => {
      try {
        const { id } = req.params as { id: number };
        const deleteAfiliadoCommand =
          req.diScope.resolve<DeleteAfiliadoCommand>("deleteAfiliadoCommand");
  ...
  app.get(
    "/afiliado/estados",
    {
      ...
      schema: {
        description:
          "Obtener todos los estados de validación de afiliados disponibles",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        response: {
          200: {
  ...
  app.get(
    "/afiliado/pendientes",
    {
      ...
      schema: {
        description:
          "Obtener lista de afiliados pendientes de aprobación (numValidacion = 1)",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        response: {
          200: {
    ...
    async (req, reply) => {
      try {
        ...
        const afiliadosPendientes = await getAfiliadosPendientes(
          userOrg0,
          userOrg1,
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosPendientes",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const afiliadosRegistrados = await getAfiliadosByStatus(
          userOrg0,
          userOrg1,
          1,
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosRegistrados",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
        ...
        querystring: {
          ...
          properties: {
            org0: {
              type: "string",
              description:
                "Código organizacional nivel 0 (requerido si es admin)",
            },
            org1: {
              type: "string",
              description:
                "Código organizacional nivel 1 (requerido si es admin)",
            },
          },
        },
        response: {
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosAprobados",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const afiliadosEnRevision = await getAfiliadosByStatus(
          userOrg0,
          userOrg1,
          3,
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosEnRevision",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const afiliadosRechazados = await getAfiliadosByStatus(
          userOrg0,
          userOrg1,
          4,
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosRechazados",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const afiliadosSuspendidos = await getAfiliadosByStatus(
          userOrg0,
          userOrg1,
          5,
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosSuspendidos",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const afiliadosCancelados = await getAfiliadosByStatus(
          userOrg0,
          userOrg1,
          6,
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadosCancelados",
          user: req.user?.sub,
        });
      }
    },
    ...
    async (req, reply) => {
      try {
        ...
        if (!id || id <= 0) {
          return reply
            .code(400)
            .send(fail("El ID del afiliado debe ser un número positivo"));
        }
        ...
        return handleAfiliadoError(error, reply, {
          operation: "getAfiliadoStatusHistory",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cambiarAStatusRegistrado",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
        ...
        body: {
          ...
          properties: {
            motivo: {
              type: "string",
              maxLength: 500,
              description: "Motivo del regreso al estado inicial",
            },
            observaciones: {
              type: "string",
              maxLength: 1000,
    ...
    async (req, reply) => {
      try {
        ...
        const resultado = await cambiarAStatusAfiliado(
          ...
          body.observaciones ||
            `Afiliado regresado al estado inicial desde estado: ${afiliadoActual.numValidacion}`,
          req.ip,
          req.headers["user-agent"],
        );
        ...
        return handleAfiliadoError(error, reply, {
          operation: "regresarAEstadoInicial",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cambiarAStatusAprobado",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cambiarAStatusEnRevision",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cambiarAStatusRechazado",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cambiarAStatusSuspendido",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cambiarAStatusCancelado",
          user: req.user?.sub,
          afiliadoId: (req.params as any)?.id,
        });
      }
  ...
  app.post(
    "/afiliado/:id/aplicar-bdisssspea-individual",
    {
      preParsing: async (req: any, _reply: any) => {
      ...
      schema: {
        description:
          "Aplicar BDIsspea a un solo afiliado. Migra sus movimientos activos a Firebird y cambia solo ese afiliado a estado 7. No modifica BitacoraAfectacionOrg global.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        params: {
          type: "object",
    ...
    async (req, reply) => {
      try {
        ...
        const userOrg0 = req.user?.idOrganica0
          ? String(req.user.idOrganica0).trim().toUpperCase().padStart(2, "0")
          : "";
        const userOrg1 = req.user?.idOrganica1
          ? String(req.user.idOrganica1).trim().toUpperCase().padStart(2, "0")
          : "";
        ...
        const command = req.diScope.resolve<AplicarBDIsspeaIndividualCommand>(
          "aplicarBDIsspeaIndividualCommand",
        );
        ...
        if (error.message === "AFILIADO_NOT_FOUND_OR_NOT_IN_ORGANICA") {
          return reply
            ...
            .send(
              fail(
                "Afiliado no encontrado o no pertenece a la orgánica del usuario",
              ),
            );
        }
        if (error.message?.startsWith("AFILIADO_STATUS_NOT_ELIGIBLE:")) {
          ...
          return reply
            ...
            .send(
              fail(
                `El afiliado no es elegible para aplicar a BDIsspea. Estado actual: ${status}. Estado permitido: 2`,
              ),
            );
        }
        if (error.message === "AFILIADO_WITHOUT_ACTIVE_MOVEMENTS") {
          return reply
            .code(400)
            .send(fail("El afiliado no tiene movimientos activos para migrar"));
        }
        return handleAfiliadoError(error, reply, {
  ...
  app.post(
    "/afiliado/aplicar-bdisssspea-lote",
    {
      preParsing: async (req: any, _reply: any) => {
        // Normalizar body ANTES de la validación del schema
        if (typeof req.body === "string") {
        ...
        body: {
          ...
          properties: {
            motivo: {
              type: "string",
              maxLength: 500,
              description: "Motivo del cambio de estado en lote",
            },
            observaciones: {
              type: "string",
              maxLength: 1000,
            ...
            properties: {
              ...
              data: {
                ...
                properties: {
                  ...
                  afiliadosCambiadosEstado: {
                    type: "number",
                    description:
                      "Cantidad de afiliados que cambiaron de estado",
                  },
                  afiliadosFallidos: {
                    type: "number",
                    description:
                      "Cantidad de afiliados que fallaron al procesar",
                  },
                  afiliadosCompletos: {
                    type: "number",
                    description: "Total de afiliados marcados como completos",
                  },
                  bitacoraActualizada: {
                    type: "number",
                    description:
                  ...
                  periodo: {
                    ...
                    nullable: true,
                    description:
                      "Periodo QQAA resuelto desde BitacoraAfectacionOrg",
                  },
                  quincena: {
                    type: "number",
                    nullable: true,
                    description:
                      "Quincena resuelta desde BitacoraAfectacionOrg",
                  },
                  anio: {
                    type: "number",
                    nullable: true,
                    description: "Año resuelto desde BitacoraAfectacionOrg",
                  },
                  quincenaId: {
                    type: "string",
                    nullable: true,
                    description:
                      "QuincenaId usada para filtrar movimientos (YYYY-QQ)",
                  },
                  afectacionId: {
                    type: "number",
                    nullable: true,
    ...
    async (req, reply) => {
      try {
        ...
        const aplicarBDIsspeaLoteCommand =
          req.diScope.resolve<AplicarBDIsspeaLoteCommand>(
            "aplicarBDIsspeaLoteCommand",
          );
        ...
        const erroresUnicos = new Map<
          string,
          { cveError: number; nomError: string; cantidad: number }
        >();
        ...
        if (resultado.afiliadosProcesados) {
          resultado.afiliadosProcesados.forEach((afiliado: any) => {
            if (
              !afiliado.exito &&
              afiliado.mensaje &&
              !afiliado.movimientos?.some((m: any) => !m.exito)
            ) {
        ...
        if (error.message?.startsWith("MOVIMIENTOS_ESTADO_NO_PERMITIDO:")) {
          ...
          return reply
            ...
            .send(
              fail(
                `No se puede finalizar la aplicacion de movimientos. Existen ${totalNoPermitidos} movimiento(s) fuera de los estados permitidos: Aprobado, Cancelado o Aplicado.`,
              ),
            );
        }
        return handleAfiliadoError(error, reply, {
  ...
  app.post(
    "/afiliado/aplicar-bdisssspea-qna",
    {
      preHandler: [requireAuth],
      schema: {
        description:
    ...
    async (req, reply) => {
      try {
        ...
        const aplicarBDIssspeaQNACommand =
          req.diScope.resolve<AplicarBDIssspeaQNACommand>(
            "aplicarBDIssspeaQNACommand",
          );
  ...
  app.get(
    "/afiliado/bitacora-accion",
    {
      preHandler: [requireAuth],
      schema: {
        description:
          "Obtener el registro completo más reciente de BitacoraAfectacionOrg para la orgánica del usuario (Entidad=AFILIADOS). Permite conocer en qué acción se encuentra actualmente el proceso.",
        tags: ["afiliado", "firebird"],
        security: [{ bearerAuth: [] }],
        querystring: {
          type: "object",
          properties: {
            ...
            periodo: {
              ...
              pattern: "^\\d{4}$",
              description:
                "Periodo QQAA opcional para consultar la bitácora de una quincena específica",
            },
          },
        },
        response: {
    ...
    async (req, reply) => {
      try {
        ...
        console.log(
          `[BITACORA_ACCION] Consultando BitacoraAfectacionOrg para orgánica ${userOrg0}/${userOrg1}${periodo ? `, periodo ${periodo}` : ""}`,
        );

        const bitacora = periodo
          ? await getBitacoraAfectacionOrgPorOrganicaYPeriodo(
              userOrg0,
              userOrg1,
              periodo,
            )
        ...
        console.log(
          `[BITACORA_ACCION] ✅ Registro obtenido - Accion: ${bitacora.accion}, Anio: ${bitacora.anio}, Quincena: ${bitacora.quincena}`,
        );
        ...
        if (error.message === "PERIODO_INVALIDO") {
          return reply
            ...
            .send(
              fail(
                "PERIODO_INVALIDO: periodo debe tener formato QQAA y quincena válida de 01 a 24",
              ),
            );
        }
        return handleAfiliadoError(error, reply, {
    ...
    async (req, reply) => {
      try {
        ...
        if (!body.afiliadoIds || body.afiliadoIds.length === 0) {
          return reply
            .code(400)
            .send(fail("Debe proporcionar al menos un ID de afiliado"));
        }
        ...
        return handleAfiliadoError(error, reply, {
          operation: "registrarAfiliadosLote",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "aprobarAfiliadosLote",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "enviarRevisionAfiliadosLote",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "rechazarAfiliadosLote",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "suspenderAfiliadosLote",
          user: req.user?.sub,
        });
      }
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        return handleAfiliadoError(error, reply, {
          operation: "cancelarAfiliadosLote",
          user: req.user?.sub,
        });
      }
    },
