...
export default async function afiliadoRoutes(app: FastifyInstance) {
  ...
  app.get('/afiliado/historial-movimientos-quincena', {
    preHandler: [requireAuth],
    schema: {
      description: 'Consulta histórico de movimientos de afiliados en SQL Server por quincena. Incluye Afiliado, AfiliadoOrg, Movimiento y TipoMovimiento.',
      ...
      querystring: {
        type: 'object',
        properties: {
          periodo: { type: 'string', pattern: '^[0-9]{4}$', description: 'Periodo formato QQAA, ejemplo 0626' },
          quincena: { type: 'integer', minimum: 1, maximum: 24, description: 'Quincena, alternativa a periodo' },
          anio: { type: 'integer', minimum: 2000, maximum: 2099, description: 'Año, alternativa a periodo' },
          org0: { type: 'string', minLength: 1, maxLength: 2, pattern: '^[A-Za-z0-9]{1,2}$', description: 'Orgánica 0 opcional; si no viene se toma del token' },
          org1: { type: 'string', minLength: 1, maxLength: 2, pattern: '^[A-Za-z0-9]{1,2}$', description: 'Orgánica 1 opcional; si no viene se toma del token' },
          buscar: { type: 'string', maxLength: 100, description: 'Búsqueda por nombre, apellidos, RFC, CURP, noEmpleado o interno' },
    ...
    try {
      ...
      const getHistorial = req.diScope.resolve<GetHistorialMovimientosQuincenaSqlQuery>('getHistorialMovimientosQuincenaSqlQuery');
      ...
      return handleAfiliadoError(error, reply, { operation: 'getHistorialMovimientosQuincena', user: req.user?.sub });
    }
  });

  app.get('/afiliado/historial-movimientos-quincena-firebird', {
    preHandler: [requireAuth],
    schema: {
      description: 'Consulta movimientos de afiliados por periodo y orgánica en Firebird. Incluye PERSONA, ORG_PERSONAL e HISTORIAL_MOVIMIENTOS_QUIN_IND.',
      ...
      querystring: {
        ...
        required: ['periodo'],
        properties: {
          periodo: { type: 'string', pattern: '^[0-9]{4}$', description: 'Periodo formato QQAA, ejemplo 0526' },
          org0: { type: 'string', minLength: 1, maxLength: 2, pattern: '^[A-Za-z0-9]{1,2}$', description: 'Orgánica 0 opcional; si no viene se toma del token' },
          org1: { type: 'string', minLength: 1, maxLength: 2, pattern: '^[A-Za-z0-9]{1,2}$', description: 'Orgánica 1 opcional; si no viene se toma del token' },
          buscar: { type: 'string', maxLength: 100, description: 'Búsqueda por interno, nombre, noEmpleado, RFC o movimiento' },
          page: { type: 'integer', minimum: 1, default: 1 },
          pageSize: { type: 'integer', minimum: 1, maximum: 500, default: 100 }
        }
      },
    ...
    try {
      ...
      const getHistorial = req.diScope.resolve<GetHistorialMovimientosQuincenaFirebirdQuery>('getHistorialMovimientosQuincenaFirebirdQuery');
      const result = await getHistorial.execute({
        org0: query.org0 || req.user?.idOrganica0 || null,
        org1: query.org1 || req.user?.idOrganica1 || null,
        periodo: query.periodo,
      ...
      return handleAfiliadoError(error, reply, { operation: 'getHistorialMovimientosQuincenaFirebird', user: req.user?.sub });
    }
  });
