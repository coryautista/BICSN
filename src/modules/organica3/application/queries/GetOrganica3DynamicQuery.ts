...
export class GetOrganica3DynamicQuery {
  ...
  async execute(query: DynamicQuery, scope: { org0: string; org1: string }, userId?: string): Promise<Organica3[]> {
    ...
    try {
      const result = await this.organica3Repo.dynamicQuery(query, scope);
