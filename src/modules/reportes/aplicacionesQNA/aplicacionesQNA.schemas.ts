...
const organicaSchema = z.string().regex(/^[A-Za-z0-9]{2}$/).describe('Clave orgánica de 2 caracteres alfanuméricos (ej: "04", "A2")');
const organicaOptionalSchema = organicaSchema.optional();
...
export const AplicacionHIPParamsSchema = z.object({
  org0: organicaOptionalSchema,
  org1: organicaOptionalSchema,
...
export const LineaCapturaParamsSchema = z.object({
  ...
  idOrg0: z.string().regex(/^[A-Za-z0-9]{1,2}$/).optional().describe('Clave orgánica nivel 0 (1-2 caracteres alfanuméricos, opcional - se usa del token si no se proporciona)'),
