...
export class ReportsRepository implements IReportsRepository {
  async getMonthlyPersonnelReport(filters: ReportFilters): Promise<MonthlyPersonnelReport[]> {
    ...
    return executeSerializedQuery((db) => {
      return new Promise<MonthlyPersonnelReport[]>((resolve, reject) => {
        db.query(sql, params, (err: any, result: any) => {
          ...
          const reports: MonthlyPersonnelReport[] = decodedResult.map((row: any) => ({
            month: filters.month,
            year: filters.year,
  ...
  async getPersonnelMovements(filters: ReportFilters): Promise<PersonnelMovement[]> {
    ...
    return executeSerializedQuery((db) => {
      return new Promise<PersonnelMovement[]>((resolve, reject) => {
        db.query(sql, params, (err: any, result: any) => {
          if (err) {
            logger.error({
              error: err.message,
          ...
          const movements: PersonnelMovement[] = decodedResult.map((row: any) => ({
            interno: row.INTERNO,
            nombreCompleto: row.NOMBRE_COMPLETO || '',
            organica0: row.CLAVE_ORGANICA_0,
            organica1: row.CLAVE_ORGANICA_1,
            tipoMovimiento: row.ACTIVO === 'A' ? 'ALTA' : 'BAJA',
            fechaMovimiento: toIsoString(row.FECHA_MOV_ALT),
