  ...
  try {
    const lock = await new sql.Request(transaction).query(`
      ...
      EXEC @Result=sys.sp_getapplock
        @Resource=N'BICSN_QNA_PHASE12_CALIDAD_MIGRATIONS',
        @LockMode=N'Exclusive',@LockOwner=N'Transaction',@LockTimeout=0;
      SELECT @Result AS LockResult;
    `);
    if (Number(lock.recordset[0]?.LockResult) < 0) throw new Error('QNA_PHASE12_MIGRATION_LOCK_UNAVAILABLE');
    ...
    if (execute) {
      ...
      console.log('QNA_PHASE12_CALIDAD_MIGRATIONS_OK');
    } else {
      await transaction.rollback();
      console.log(JSON.stringify(evidence, null, 2));
      console.log('QNA_PHASE12_CALIDAD_MIGRATIONS_DRY_RUN_OK');
    }
  } catch (error) {
