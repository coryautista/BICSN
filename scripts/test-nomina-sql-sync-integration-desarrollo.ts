...
const rows: NominaAplicacionQnalRegistroParsed[] = [1, 2].map((numeroLinea) => ({ numeroLinea, lote: '2398001', tipoRegistro: '2', clavePersonal: `SYN${marker}${numeroLinea}`, rfc: `SYN${marker.slice(0, 5)}${numeroLinea}T0`, nombreAfiliado: `SINTETICO ${numeroLinea}`, aportacionAfiliadoFondoAhorro: 10 + numeroLinea, aportacionEntidadFondoAhorro: 20 + numeroLinea, aportacionAfiliadoEBI: null, aportacionEntidadEBI: null, baseCotizacionSueldo: 1000 + numeroLinea, baseCotizacionQuinquenios: 5 + numeroLinea, sueldoMensual: 2000 + numeroLinea, ayudasMensuales: null, quinqueniosMensual: 10 + numeroLinea, descuentoPrestamoCortoPlazo: 30 + numeroLinea, descuentoPrestamoHipotecario: 40 + numeroLinea, fechaMovimiento: new Date('2098-12-01T00:00:00Z'), descuentoPrestamoMedianoPlazo: null, descuentosOtros: null, cair: 50 + numeroLinea, cairVoluntario: null, fechaRegistro: new Date(), diasLaborados: 15, layoutVersion: '20', lineaOriginal: `SYNTHETIC-${marker}-${numeroLinea}` }));
...
try {
  ...
  const details = await pool.request().input('Carga',sql.BigInt,cargaId).query('SELECT LineaNumero,LineaOriginal,RFC,ClavePersonal,QuinqueniosMensual,DescuentoPrestamoCortoPlazo,DescuentoPrestamoHipotecario,CAIR,DiasLaborados FROM dbo.NominaAplicacionQnalDetalle WHERE CargaId=@Carga ORDER BY LineaNumero');
  assert.deepEqual(details.recordset.map(r=>({ linea:Number(r.LineaNumero),original:r.LineaOriginal,rfc:r.RFC,clave:r.ClavePersonal,quinquenios:Number(r.QuinqueniosMensual),pcp:Number(r.DescuentoPrestamoCortoPlazo),hip:Number(r.DescuentoPrestamoHipotecario),cair:Number(r.CAIR),dias:Number(r.DiasLaborados) })),rows.map(r=>({ linea:r.numeroLinea,original:r.lineaOriginal,rfc:r.rfc,clave:r.clavePersonal,quinquenios:r.quinqueniosMensual,pcp:r.descuentoPrestamoCortoPlazo,hip:r.descuentoPrestamoHipotecario,cair:r.cair,dias:r.diasLaborados })));
